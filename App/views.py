    ...
    def post(self,request):
        if request.method=='POST':
            ...
            if form.is_valid():
                ...
                uploaded_doc = request.FILES['image']
                print(uploaded_doc)
                names = str(uploaded_doc).split(".")
                formats = names[-1]

                if formats in ['odt','doc','docs','docx']: 
                    model = Uploads(image=request.FILES['image'], name=str(num))
                    model.save()
                    return render(request, self.template_name,{'form':form})
                else:
                    return render(request,'index.html',{'form':form,'message':"please use only these formats (odt,doc,docs,docx)"})
        else:
            form=UploadForm()
        return render(request,'index.html',{'form':form})

class Comment(TemplateView):
    template_name = 'upload_comment.html'
    def post(self,request):
        if request.method=='POST':
            comment=request.POST['comment']
            name = request.POST['name']
            model = Comments(name=name, comment=comment)
            model.save()
